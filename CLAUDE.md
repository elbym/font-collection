...
- `font-display: swap` is set globally in `_font-mixin.scss`. Specimen pages eagerly load their font via `<link>` in `<head>`, so FOUT is minimal. Lazy-loaded card fonts use `swap` intentionally — the IntersectionObserver loads CSS 300px ahead of the viewport.
