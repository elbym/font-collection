...
- The top bar (`src/_includes/topbar.njk`, included by `base.njk` above the header) has one button that cycles **system → light → dark**. `system` is the default.
- State: `localStorage['theme']` (`light` | `dark`; absent = system), mirrored onto `<html data-theme>`. A pre-paint inline script in `base.njk`'s `<head>` applies it before first render; `src/js/theme-toggle.js` handles the button and updates its label.
- SCSS never writes bare `@media (prefers-color-scheme: dark)` for theme colors any more — use the mixins in `src/scss/_includes/_theme.scss`: `theme.dark-root` (for `:root` CSS-var overrides / top-level rules) and `theme.dark` (called from inside a selector, appends `&`). Each emits both `@media (prefers-color-scheme: dark) :root:not([data-theme="light"])` **and** `:root[data-theme="dark"]`, so system and forced modes both work. Consumers: `_base/_colors.scss`, `_cards.scss`, `styles.scss`.
- `styles.scss`'s second `.cmyk_*` dark block is still a plain media query — it only re-sets `var()`-based gradients that already flip via the token overrides, so forced-dark needs nothing there.
